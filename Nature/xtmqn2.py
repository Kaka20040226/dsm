...
class Net(nn.Module):
    def __init__(self, input_dim, output_dim, task_num, provider_num, net,device="cpu", D=None): 
        ...
        for i in range(task_num):
            new_net = nn.Sequential(
                nn.Linear(provider_num+output_dim+2 * task_num * provider_num + task_num + 9 * provider_num + 1, provider_num+output_dim).to(device),
                nn.ReLU(),
                nn.Linear(provider_num+output_dim, provider_num+output_dim).to(device),
                nn.ReLU(),
    ...
    def forward(self,x,pos,p,prehc, AP, charData):
        x = self.attention(x)
        ...
        for ap in AP:
            ans = self.taskL[ap](torch.cat([x,out,charData],dim=1))
...
class network:
    ...
    def available_P(self, p):
        ...
        for c in p.children:
            ans.append(c.loc)
        return ans
    def forward(self, x, point, p, prehc, charData):
        y, hc = self.model.forward(x, point, p, prehc, self.available_P(p), charData)
        return y, hc
    ...
    def proceed(self, x, point): # calculate the choice of this step
        ...
        y, hcV  = self.forward(x, pos, point, point.hc, self.charData)
    ...
    def run_step(self, startpoint, state=0): # do one search with search, calculate objective value, and then calculate loss
        ...
        for p in self.points:
            ...
            p.sense = "And"
            p.time = None
        calcuated, x = self.search(startpoint)
        objv, obj = self.objv(x[-1])
        
        return calcuated, x, objv, obj
